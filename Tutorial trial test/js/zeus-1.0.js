...
var openNewDate = null;
var dateOnDayView = null;
...
; (function ($, window, document, undefined) {
    ...
    $.zeusValidate.addMessage = function (message, startingContainer, clear, type) {
        ...
        if (type == undefined || (type != $.zeusValidate.messageTypes.Error && type != $.zeusValidate.messageTypes.Information && type != $.zeusValidate.messageTypes.Warning && type != $.zeusValidate.messageTypes.Success)) {
            type = 'error';
        }
        ...
        message = $.zeusValidate.replaceAll(message, '..', '.');
        ...
        if (type == $.zeusValidate.messageTypes.Error) {
            var container = startingContainer.find("#validation-error-summary"),
            ...
            $("<li />").append(message).appendTo(list);
            ...
            if (type == $.zeusValidate.messageTypes.Success) {
                msgClass = 'msgGood';
                alertClass='alert-success';
                header = 'Success';
            } else if (type == $.zeusValidate.messageTypes.Warning) {
                msgClass = 'msgWarn';
                alertClass='alert-warning';
                header = 'Warning';
            }

            var container = startingContainer.find("section." + msgClass);

            // Check to see whether the container exists, if not, add it before the error container
            if (!container.length) {
                var errorContainer = startingContainer.find("#validation-error-summary");

                container = $('<section class="' + msgClass + ' alert ' + alertClass + ' fade in m-b-15"><span tabindex="0" class="close" aria-label="Close message below" data-dismiss="alert"><span aria-hidden="true">×</span></span><h2>' + header + '</h2><ul><li></li></ul></section>');

                errorContainer.before(container);
            }
    ...
    $.zeusValidate.addError = function (message, startingContainer, clear) {
        $.zeusValidate.addMessage(message, startingContainer, clear, $.zeusValidate.messageTypes.Error);
    };

    $.zeusValidate.addSuccess = function (message, startingContainer, clear) {
        $.zeusValidate.addMessage(message, startingContainer, clear, $.zeusValidate.messageTypes.Success);
    };

    $.zeusValidate.addInformation = function (message, startingContainer, clear) {
        $.zeusValidate.addMessage(message, startingContainer, clear, $.zeusValidate.messageTypes.Information);
    };

    $.zeusValidate.addWarning = function (message, startingContainer, clear) {
        $.zeusValidate.addMessage(message, startingContainer, clear, $.zeusValidate.messageTypes.Warning);
    };
    ...
    var fullDataTypes = {
        ...
        GraphType: 'data-zgt',
        GraphXaxisLabel: 'data-zgxal',
        GraphXaxisMin: 'data-zgxamin',
        GraphXaxisMax: 'data-zgxamax',
        WidgetContext: 'data-zwc',
        WidgetView: 'data-zwv',
        WidgetDataContext: 'data-zwdc',
        WidgetDefaultLayout: 'data-zwdl',
        CalendarData: "data-zcald",
        CalendarDefaultView: "data-zcdv",
        CalendarDisplayViews: "data-zcv",
        CalendarMinDateSelectable: "data-zcmin",
        CalendarMaxDateSelectable: "data-zcmax",
        CalendarResourceLabelText: "data-zcrl",
        CalendarCacheRangeDays: "data-zccrd",
        CalendarGetEventsUrl: "data-zceu",
        CalendarGetResourcesUrl: "data-zcru",
        CalendarCategoryCreateUrl: "data-zccu",
        CalendarUseLastView: "data-zclv",
        CalendarUseLastDate: "data-zcld",
        PersistedDay: "data-zcpd",
        CalendarShowWeekends: "data-zcwe",
        AjaxRoutes: 'data-zr',
        ...
        DocumentStoreWebApiSubmitTypes: 'data-dswast',
        FileSize: 'data-zfs',
    };
...
//                window.sessionStorage['callback.last.window.ajaxOptions'] = JSON.stringify(ajaxOptions);
            ...
            /*var callback = function () {
					...
					var storedAjaxOptions = window.sessionStorage['callback.last.window.ajaxOptions'];
					var ajaxOptions = storedAjaxOptions != undefined ? $.parseJSON(storedAjaxOptions) : null;
        ...
        showModalDialogElement: function(modal) {
            ...
            //focus on control
            setTimeout(function () {
                modal.find('*').filter(':input:not(input[type=button]):not(button):visible:first').focus();
            }, 1000);
        },
        ...
        contractmodal: function () {
            ...
            root.find('[' + fullDataTypes.ContractsForContractModal + ']').on('click.zeus-contractmodal', function (e) {
                e.preventDefault();
                ...
                // Ensure breadcrumb is closed before showing contract modal
                $('#breadcrumb').smartmenus('menuHideAll');
                ...
                modal.find('.modal-body a').each(function() {
                    ...
                    contractlink.on('click.zeus-contractmodallink', function (event) {
                        ...
                        if (contractIndex != -1) {
                            window.location.href = menuhref.substr(0, contractIndex) + contractReplacement + menuhref.substr(contractIndex + currentContractFind.length);
                        }
                    });
        ...
        breadcrumb: function() {
            ...
            $(breadcrumb.find('li')[0]).before($('<li class="ignore"><a href="javascript:;" tabindex="-1">You are here: </a></li>'));

            if (breadcrumb.length) {

                breadcrumb.smartmenus({
                ...
                // Make 'Related Links' appear on the right
                var relatedLinks = breadcrumb.find('a:contains(Related Links)').parent('li');
                if (relatedLinks.length) {
                    for (var i = 0; i < relatedLinks.length; i++) {
                        if ($(relatedLinks[i]).parent().is('#breadcrumb')) {
                            $(relatedLinks[i]).addClass('pull-right sm-related');
                            break;
                        }
                    }
                }

                breadcrumb.on('mousedown.breadcrumb-autoclose', function (event) {
                    event.originalEvent.breadcrumbinsidetrigger = true;
                });
                $(document).on('mousedown.breadcrumb-autoclose', function (event) {
                    if (!event.originalEvent.breadcrumbinsidetrigger) {
                        breadcrumb.smartmenus('menuHideAll');
                    }
                });
                $(document).on('keydown.breadcrumb-autoclose', function (event) {
                    if (event.which == 27) { // Close on escape key
                        breadcrumb.smartmenus('menuHideAll');
                    }
                });
        ...
        menufiltering: function () {
            ...
            // Current contract only in menu and breadcrumb
            root.find('.contract-menu-filter').on("select.menu-filtering", function () {
                ...
                root.find('#breadcrumb li.not-cc-valid').addClass('hide');
            ...
            .on("deselect.menu-filtering", function () {
                ...
                root.find('#breadcrumb li.not-cc-valid').removeClass('hide');
                showHideHigherLevelMenus();
            })
/*            .each(function () {
...
/*            .each(function () {
            ...
            var adjustFilterIcon = function (text) {
                if (text != '') {
                    root.find('div.search-menu-filter').removeClass('has-feedback');
                    root.find('.search-menu-filter-icon').addClass('hidden');
                } else {
                    root.find('div.search-menu-filter').addClass('has-feedback');
                    root.find('.search-menu-filter-icon').removeClass('hidden');
                }
            };
            ...
            root.find('div.search-menu-filter input').on('keydown keyup', function () {
                var text = $.zeusValidate.getValueFromInput($(this)) || '';
                ...
                root.find('a.expand-collapse-menu-filter').trigger('select');
            });
            root.find('div.search-menu-filter input').on('focusout', function () {
                adjustFilterIcon('');
            });
        ...
        primary: function () {
            ...
            root.find('input[type="text"], input[type="email"], input[' + fullDataTypes.Switcher + '="true"], input[type="checkbox"], input[type="radio"]').on('keypress.rhea-primary', function (e) {
                var property = $(this);
            ...
            $('#main_form').on('keypress.rhea-primary', function (e) {
                // On ENTER and not readonly, submit closest primary button, otherwise allow browser to decide
                if (e.which == 13) {
                    ...
                    if (element != undefined && element.length) {
                        ...
                        isTextArea = $(document.activeElement).is('textarea');
                    }

                    var primary = $(this).find('.btn-primary').not(':hidden')[0];

                    if (!isTextArea && primary != undefined) {
                        primary.click();
                        e.preventDefault();
                        return false;
                    }
            ...
            /*
             root.find('fieldset').on('keypress.rhea-primary', function(e) {
        ...
        maxlength: function () {
            ...
            root.find('[maxlength]').not('[data-val-range]').each(function () {
                ...
                property.on('focus resize', function (e) {
        ...
        dirtycheck: function () {
            ...
            root.find('button').each(function () {
                    if (!$(this).hasEvent('click.rhea-ignoredirty')) {
                        $(this).on('click.rhea-ignoredirty', function () {
        ...
        _checkWebApiResponseForErrors: function(xhr, returnErrors) {
            // Similar error handling to EmploymentWebApiClient

            function AddErrors(errors) {
                if (returnErrors || errors == undefined || !$.isArray(errors) || errors.length == 0) {
                    return;
                }
        ...
        senddisabled: function () {
            ...
            root.find('button[name="submitType"]').on('click.lastclicked', function () {
                var button = $(this);
            ...
            if (forms.length <= 0) {
                root.find('button[name="submitType"]').on('click.rhea-senddisabled', function () {
                    var button = $(this);
                ...
                forms.each(function () {
                    ...
                    form.find('button[name="submitType"]').on('click.rhea-senddisabled', function () {
                        var button = $(this);
                    ...
                    form.submit(function (e) {
                            ...
                            if (!skipconfirm && url != undefined) {
                                ...
                                if (parameters.length > 0) {
                                    for (var i = 0; i < parameters.length; i++) {
                                        ...
                                        // Check if button is inside a grid
                                        var tr = button.parents('tr');

                                        if (tr.length && results[key] == "") {
                                            var buttonValue = button.val();
                                            
                                            if (buttonValue != undefined && buttonValue.length && buttonValue.indexOf('=') != -1) {
                                                buttonValue = buttonValue.split('=')[1];

                                                results[key] = buttonValue;
                                            }
                                        }
                                    }
                                }
                                ...
                                $.ajax({
                                    ...
                                    if (confirmationModal.length == 0) {
                                        ...
                                        confirmationModal.find('.modal-footer a.btn-primary').on('click.zeus-modal-yes', function () {
                                        ...
                                        confirmationModal.find('.modal-footer a').on('click.zeus-modal-dismiss', function () {
                        ...
                        var callDocumentStoreWebApi = documentStoreWebApiUrl != undefined;

                        if (callDocumentStoreWebApi) {
                            var submitTypes = form.data(dataTypes.DocumentStoreWebApiSubmitTypes);

                            if (submitTypes != undefined) {
                                var submitType = button.attr('value');

                                // Only call if the submit type is one of the defined types
                                callDocumentStoreWebApi = submitTypes.indexOf(submitType) > -1;
                            }
                        }
                        ...
                        if (callDocumentStoreWebApi)
                        {
                            ...
                            var spinnerModal = $('<div class="body-loader"><span class="spinner-small progress-spinner"></span><span class="progress-text"></span></div>');

                            $('body').prepend(spinnerModal);
                            ...
                            */
                                ...
                                xhr: function () {
                                    ...
                                    xhr.upload.addEventListener("progress", function (evt) {
                                        if (evt.lengthComputable) {
                                            var progress = spinnerModal.find('.progress-text');
                                            var percentComplete = parseInt((evt.loaded / evt.total) * 100);

                                            progress.html(percentComplete + '%' + '<br /><br />' + 'Uploading...');
                                            
                                            if (percentComplete === 100 && !uploadDone) {
                                                progress.html(percentComplete + '%' + '<br /><br />' + 'Waiting...');
                                                uploadDone = true;
                                            }
                                        }
                                    }, false);
                                    ...
                                    xhr.addEventListener("progress", function (evt) {
                                        if (evt.lengthComputable) {
                                            ...
                                            progress.html(percentComplete + '%' + '<br /><br />' + 'Processing...');
                                            
                                            if (percentComplete === 100) {
                                                progress.html(percentComplete + '%' + '<br /><br />' + 'Done');
                                            }
                                        }
                                    }, false);
                            ...
                            $.ajax(ajaxOptions).done(function (data, textStatus, request) {
                                ...
                                if (response != undefined && response.Data != undefined && response.Data.Guid != undefined) {
                                    $.zeusValidate.addSuccess('Document uploaded successfully (' + response.Data.Guid + ').');
                                } else {
                                    $.zeusValidate.addSuccess('Document uploaded successfully.');
                                }

                                finished();
                            })
                            .fail(function (xhr, status, data) {
                                $rhea._checkWebApiResponseForErrors(xhr);
            ...
            root.find('.rhea-actionif').each(function () {
                ...
                if (dependentProperty.length > 1) {
                    ...
                    for (var i = 0; i < dependentProperty.length; i++) {
                        $(dependentProperty[i]).on('change.rhea-actionif', actionIfChange);
                    }
                } else {
                    dependentProperty.on('change.rhea-actionif', actionIfChange);
                }
            });
        },
            ...
            var prepareContingentIf = function (property, contingentDataType) {
                ...
                var setupDependentChangeHandler = function (contingentType, contingent, contingentTypeHandler) {
                    ...
                    if (dependentProperty.length > 1) {
                        // Bind to each radio button element
                        for (var i = 0; i < dependentProperty.length; i++) {
                            $(dependentProperty[i]).on('change.zeus-trigger' + contingentType, contingentTypeHandler);
                        }
                    } else {
                        dependentProperty.on('change.zeus-trigger' + contingentType, contingentTypeHandler);
                    }
                };
                ...
                if (contingentChangeHandler == requiredIfChangeHandler) {
                    ...
                    property.on('load.zeus-trigger-requiredif', requiredIfChangeHandler);
                }
            };
        ...
        ajaxloadif: function () {
            ...
            root.find('.rhea-ajaxloadif').each(function () {
                ...
                if (dependentProperty.length > 1) {
                    // Bind to each radio button element
                    for (var i = 0; i < dependentProperty.length; i++) {
                        $(dependentProperty[i]).on('change.rhea-ajaxloadif', ajaxLoadIfChange);
                    }
                } else if (dependentProperty.length == 1) {
                    dependentProperty.on('change.rhea-ajaxloadif', ajaxLoadIfChange);
                }
                
                property.on('load.rhea-ajaxloadif', ajaxLoadIfChange);
        ...
        gst: function () {
            ...
            root.find('.rhea-gst').each(function () {
                ...
                if (dependentProperty.length > 1) {
                    // Bind to each radio button element
                    for (var i = 0; i < dependentProperty.length; i++) {
                        $(dependentProperty[i]).on('change.rhea-gst', gstChange);
                    }
                } else {
                    dependentProperty.on('change.rhea-gst', gstChange);
                }
            });
        },
        ...
        age: function () {
            ...
            root.find('.rhea-age').each(function () {
                ...
                dependentProperty.on('change.rhea-age', function () {
        ...
        copy: function () {
            ...
            root.find('.rhea-copy').each(function () {
                ...
                for (var i = 0; i < dependentProperties.length; i++) {
                    ...
                    dependentProperty.on('change.rhea-copy', function () {
        ...
        relativedate: function () {
            ...
            root.find('.rhea-relativedate').each(function () {
                ...
                dependentProperty.on('change.rhea-relativedate', function () {
        ...
        datetimepicker: function () {
            ...
            root.find('.datetime').each(function () {
                ...
                dateContainer.find('.fordate').on('click', function () {
                    if (dateInput.attr('disabled') == undefined && dateInput.attr('readonly') == undefined) {
                        dateInput.focus();
                    }
                });
                
                dateInput.on('show.zeus-datepicker', function () {
                ...
                timeContainer.find('.fortime').on('click', function () {
                ...
                dateInput.on('input.track', function () {
                    dateInput.data('currentVal', dateInput.val());
                });
                dateInput.on('focusout.track', function () {
                    setTimeout(function () {
                ...
                dateInput.on('change.date focusout.date', function () {
                ...
                timeInput.on('change.time focusout.date', function () {
                    var container = $(this).parent();
                ...
                hiddenInput.on('focus.datetime', function() {
                    dateInput.focus();
                });
            });
        ...
        datepicker: function () {
            ...
            root.find('.date').each(function () {
                ...
                container.find('.input-group-addon').on('click', function () {
                ...
                input.on('show.zeus-datepicker', function () {
                    // Hide datepicker if readonly
                ...
                input.on('input.track', function () {
                    input.data('currentVal', input.val());
                });
                input.on('focusout.track', function () {
                    setTimeout(function () {
                ...
                input.on('change.date focusout.date', function () {
        ...
        ajaxproperty: function () {
            ...
            root.find('[' + fullDataTypes.PropertyNameForAjax + ']').each(function() {
                ...
                // Handle anchor
                if (element.is('a')) {
                    element.on('click.zeus-ajaxproperty', function(e) {
                        e.preventDefault();
            ...
            root.find('[' + fullDataTypes.PropertyNamesWithAjaxLoadToTrigger + ']').each(function() {
                ...
                if (element.is('a')) {
                    ...
                    element.on('click.zeus-ajaxproperties', function (e) {
                        e.preventDefault();
        ...
        paged: function () {
            ...
            root.find('.rhea-paged').each(function () {
                ...
                property.on('click.rhea-paged', function (e) {
        ...
        historypin: function () {
            ...
            root.find('a[data-click="theme-panel-expand"]').on("click", function (event) {
                ...
                var themePanel = self.closest('.theme-panel');
                if (text.match(/^close/) == null) {
                    // close all other open quickfinds before opening this one
                    root.find('a[data-click="theme-panel-expand"]').each(function () {
                        var other = $(this);
                        var otherReaderSpan = other.find('.readers');
                        var otherText = otherReaderSpan.text();
                        var otherThemePanel = other.closest('.theme-panel');
                        if (otherText.match(/^close/)) {
                            other.click();
                        }
                    });

                    themePanel.css('position', 'absolute'); // When zoomed to 200% unable to view recently accessed records as the position is Fixed, hence changing it to absolute.
                    ...
                    var actualTop = themePanel.css('top');
                    self.attr('actualTop', actualTop);
                    var newTop = parseInt(actualTop) + pageContainer.scrollTop();                    
                    themePanel.css('top', newTop);
                    window.setTimeout(function () {
                        var quickfindBox = themePanel.find('.qf-method-search .amc-box');
                        if (quickfindBox.length) {
                            quickfindBox.focus();
                        } else {
                            quickfindBox = themePanel.find('form input');
                    ...
                    themePanel.css('top', actualTop);
                    themePanel.css('position', 'fixed'); // You can still see the hidden content with position abolute, so we change it to fixed when panel is closed.
                    window.setTimeout(function () {
        ...
        menupin: function () {
            ...
            root.find('.menu-pin').each(function () {
                var pin = $(this);

                var targetAction = pin.data(dataTypes.ActionName);
                var targetController = pin.data(dataTypes.ControllerName);
                var targetArea = pin.data(dataTypes.AreaName);
                ...
                pin.on('click.zeus-pin', function (e) {
                    ...
                    var pinOrUnpin = function () {
                        pin.toggleClass('pinned');
                        // Update the hidden text for screen-reader.
                        var readersSpan = $(pin).find('span');
                        if (pin.hasClass('pinned')) {
                            // Push to top of list
                            parentItem.prependTo(parentList);
                            // Update url
                            pin.data(dataTypes.Url, url.replace('Pin', 'Unpin'));
                            readersSpan.text('Unpin this page from My Menu');

                            root.find('ul.pinned-nav li.active').removeClass('hidden');

                            var myPinnedList = root.find('ul.pinned-nav').parent();
                            myPinnedList.addClass('active');
                            myPinnedList.removeClass('hidden');
                        } else {
                            // Push to bottom of list
                            parentItem.appendTo(parentList);
                            // Update url
                            pin.data(dataTypes.Url, url.replace('Unpin', 'Pin'));
                            readersSpan.text('Pin this page to My Menu');

                            root.find('ul.pinned-nav li.active').addClass('hidden');
                            var myPinnedList = root.find('ul.pinned-nav').parent();
                            myPinnedList.removeClass('active');

                            if (myPinnedList.find('ul li:not(.sidebar-subnav-header)').length == myPinnedList.find('ul li.hidden').length) {
                                myPinnedList.addClass('hidden');
                            } else {
                                myPinnedList.removeClass('hidden');
                            }
                        }
                    ...
                    $.ajax({
                        ...
                        // Revert UI as it didn't work
                        pinOrUnpin();

                        var action = pin.hasClass('pinned') ? 'unpin from' : 'pin to';
                        $.zeusValidate.addError('Error occurred while trying to ' + action + ' menu.');
                        locked = false;
                    });
                });
            });
        ...
        multiplegridselect: function () {
            ...
            root.find('.rhea-multiple-select-grid').each(function () {
                ...
                property.on('click.rhea-mutiple-select-grid', function () {
        ...
        crn: function () {
            ...
            $('[data-val-crn]').on('change.rhea-crn', function () {
                this.value = this.value.toUpperCase();
            });
            
            $('[data-val-crn]').on('keyup.rhea-crn', function () {
        ...
        tooltipposition: function () {
            ...
            var changeVisibility = function (property, isErrorTip) {
                ...
                if ($(span).hasClass('visible')) {
                    ...
                    property.removeClass('re-adjust');
                } else {
                    property.addClass('re-adjust');
                    $(span).addClass('visible');
                    //checkPosition(property); // No need to check the position of hint/error tip as we have removed its position.
                }
            };
            ...
            root.find('.errorTip').each(function () {
                ...
                var isErrorTip = true;
                property.on('click.rhea-tooltipposition', function () { changeVisibility(property, isErrorTip); });
                property.on('onenter.rhea-tooltipposition', function () { changeVisibility(property, isErrorTip); });
                // We will no longer hide tool tips on focus out.
                //property.on('focusout.rhea-tooltipposition', function () { hideTips(); });
            });
        ...
        inlinegrid: function() {
            ...
            root.find('input[' + fullDataTypes.GridInlineEditedState + ']').each(function() {
                ...
                editedState.on('checkstate', function() {
                    // Get current state
                ...
                inputs.each(function() {
                    ...
                    input.on('change.inlinegrid', function() {
        ...
        ajaxgrid: function () {
            ...
            root.find('a[' +fullDataTypes.AjaxGridModal + ']').each(function () {
                ...
                link.on('click.ajaxgridmodal', function (e) {
                    e.preventDefault();
                    ...
                    $.ajax({
                        ...
                        */
                        ...
                        var prepareModal = function (html) {
                            ...
                            gridModal.find('.modal-body button').not('ul.amc-selected-choices li button').not('button[data-zbcl]').on('click.zeus-modal-btn', handleModal);
                        };
                        ...
                        var handleModal = function (event) {
                            ...
                            // Check if this is an Ajax document upload to the Document Store Web API
                            var form = $(gridModal.find('form')[0]);

                            // Check if form is valid
                            var valid = /^true$/i.test(button.attr(fullDataTypes.SkipValidation)) || form.valid();

                            if (!valid) {
                                return;
                            } else {
                                // Ensure error summary is cleared
                                var modalErrorContainer = gridModal.find("#validation-error-summary");

                                modalErrorContainer.addClass('validation-summary-valid')
                                                   .addClass('noErrors')
                                                   .removeClass('validation-summary-errors')
                                                   .removeClass('alert')
                                                   .removeClass('alert-danger');

                                modalErrorContainer.find('ul').empty();
                            }
                            ...
                            // Lock modal with loading spinner
                            ...
                            var spinnerModal = $('<div class="modal-loader"><span class="spinner-small"></span><span class="progress-text"></span></div>');
                            ...
                            var executePostCall = function (guid, id) {
                                // Check for deep nested view model
                                // Find out all parent types and property name in parent values
                                var closestContainer = group.parent().closest('.iscontainer');
                                ...
                                // Include selection type
                                headers[headerTypes.SelectionType] = grid.data(dataTypes.SelectionType);
                                ...
                                var dataToPost;
                                if (guid != undefined) {
                                    dataToPost = 'guid=' + guid + '&submitType=' + button.attr('value');
                                    if (id != undefined) {
                                        dataToPost += '&id=' + id;
                                    }
                                } else if (gridForm.find('input[type="file"]').length == 0) { // use regular query string
                                ...
                                $.ajax(ajaxOptions).done(function (data, textStatus, request) {
                                    ...
                                    if (type == ModalType.Delete) {
                                        ...
                                        if (newContent.data(dataTypes.AjaxGridRow)) {
                                            ...
                                            styleEditedRow(tr);
/*
                                            var gridTable = link.closest('table');
                                            if (gridTable.attr(fullDataTypes.GridFilterStatus) == "true") {// check if filtering is allowed client-side, if so apply filter again.
                                                gridTable.trigger($.fn.filterTable.defaultOptions.contentChangedEvent);
                                            }
                                            $rhea.responsivetables(newContent);
*/
                            ...
                            if (callDocumentStoreWebApi) {
                                ...
                                spinnerModal.find('.spinner-small').addClass('progress-spinner');
                                ...
                                $.ajax(ajaxOptions).done(function (data, textStatus, request) {
                                    // Web API has completed successfully, now call Zeus POST action with GUID to get the new grid row HTML
                                    var response = $.parseJSON(request.responseText);
                                    if (response != undefined && response.Data != undefined && response.Data.Guid != undefined) {
                                        //$.zeusValidate.addSuccess('Document uploaded successfully (' + response.Data.Guid + ').');
                                        executePostCall(response.Data.Guid, response.Data.Id);
                                    } else {
                                        // Shouldn't get here if the Web API call was successful, but if we're here we need to indicate it worked even though we don't have the GUID to prove it
                                        //$.zeusValidate.addSuccess('Document uploaded successfully.');
                                        var tr = link.closest('tr')[0];
                                        styleEditedRow(tr);
                                        $rhea.dismissModalDialogElement(gridModal);
                                ...
                                .fail(function (xhr, status, data) {
                                    var errors = $rhea._checkWebApiResponseForErrors(xhr, true);
                                    ...
                                    if (errors.length) {
                                        ...
                                        clear = true;
                                        for (var i = 0; i < errors.length; i++) {
                                            $.zeusValidate.addError(errors[i], $('.modal'), clear);
                                            clear = false;
                                        }
                                    } else {
                                        // Unexpected non-Web API error response
                                        $.zeusValidate.addError('Error occurred while uploading.', $('.modal'), true);
                                    }
            ...
            root.find('a[' + fullDataTypes.AjaxGridInline + ']').each(function () {
                ...
                link.on('click.ajaxgridinline', function(e) {
                    e.preventDefault();
        ...
        reset: function () {
            ...
            root.find('input[type="reset"]').each(function () {
                ...
                resetButton.on('click.rhea-reset', function (e) {
        ...
        checkboxesadditions: function () {
            $(".check-box-list", this.element).each(function (index) {
                ...
                input.on("change.rhea-checkall", function (event) {
        ...
        flotGraphs: function () {
            ...
            root.find(".zeus-graph-table", $rhea.element).each(function () {
                ...
                function renderGraph(graph, pieindex) {
                    ...
                    var xaxisTicks = graph.attr(fullDataTypes.GraphXaxisLabel);
                    var xaxisTicksMin = graph.attr(fullDataTypes.GraphXaxisMin);
                    var xaxisTicksMax = graph.attr(fullDataTypes.GraphXaxisMax);

                    if (xaxisTicks && $.zeusValidate.isJSON(String(xaxisTicks))) {
                        var tickModel = [];
                        var tickJSON = JSON.parse(String(xaxisTicks));
                        for (var key in tickJSON) {
                            if (tickJSON.hasOwnProperty(key)) {
                                tickModel.push([key, tickJSON[key]]);
                            }
                        }

                        if (xaxisTicksMin && xaxisTicksMax && parseInt(xaxisTicksMin) > 0 && parseInt(xaxisTicksMax) > 0) {
                            $.extend(options, { xaxis: { ticks: tickModel, min: parseInt(xaxisTicksMin), max: parseInt(xaxisTicksMax) } });
                        }
                        else if (xaxisTicksMax && parseInt(xaxisTicksMax) > 0) {
                            $.extend(options, { xaxis: { ticks: tickModel, max: parseInt(xaxisTicksMax) } });
                        } else {
                            $.extend(options, { xaxis: { ticks: tickModel } });
                        }
                    }
//                    $.extend(options, {
...
//                    })
                    ...
                    if (graphType == "pie") {
                        ...
                        var tickModel = [];
                        var counter = 0;
                        var numColumns = table.find('tr').last().find('td:not(.zeus-graph-axis-label)').length; // This is the length of one row only, but since the tables are all rectangular, this should be okay.
                        if (numColumns > 1) { // Multi bars
                            ...
                            table.find('tr').each(function (rowNum) {
                                ...
                                var key = $(this).find('th').text();
                                $(this).find('td:not(.zeus-graph-axis-label)').each(function (colNum) {
                            ...
                            table.find('tr').each(function (rowNum) {
                                var key = $(this).find('th').text();
                                var value = parseFloat($(this).find('td:not(.zeus-graph-axis-label)').text());
                                dataModel.push({
                                    label: key,
                                    data: [[value, counter]],
                                });
                        ...
                        tipFormatter = function (tip, item) {
                            ...
                            var seriesValue = (item.series.data[item.dataIndex][0].toString().replace(/(\d)(?=(\d{3})+(?!\d))/g, "$1,"));
                            var xAxisTicks = item.series.xaxis.ticks;
                            if (xAxisTicks && xaxisTicks) {
                                // Only applicable Graphs with custom X axis labels specified.
                                var closestItemIndex = 0;
                                var diff = null;
                                // Check if the seriesValue matches exactly with the seriesLabel.
                                for (var i = 0; i < xAxisTicks.length; i++) {
                                    if (xAxisTicks[i].v === parseInt(seriesValue)) {
                                        seriesLabel = xAxisTicks[i].label;
                                        break;
                                    } else {
                                        // If not then, get the next nearest label from the labelList.
                                        if (!diff || Math.abs(xAxisTicks[i].v - parseInt(seriesValue)) < diff) {
                                            diff = Math.abs(xAxisTicks[i].v - parseFloat(seriesValue));
                                            closestItemIndex = i;
                                        }
                                    }
                                }
                                if (seriesLabel == null) {
                                    for (var j = 0; j < xAxisTicks.length; j++) {
                                        // Do the rounding and then check if there's a matching label.
                                        if (xAxisTicks[j].v === Math.round(parseFloat(seriesValue))) {
                                            seriesLabel = xAxisTicks[j].label;
                                            break;
                                        }
                                    }
                                    if (seriesLabel == null && xAxisTicks.length > closestItemIndex) {
                                        // If not then, get the next nearest label from the labelList.
                                        seriesLabel = xAxisTicks[closestItemIndex].label;
                                    }
                                }
                                if (seriesLabel) {
                                    tip.text(item.series.label + ': ' + seriesLabel);
                                }
                            }
                            else {
                                tip.text(item.series.label + ': ' + seriesValue);
                            }
                        };
                        postDataCreator = function (item) {
                            return JSON.stringify({
                    ...
                    graph.on("plotclick.flotgraphs", function (event, pos, item) {
                        ...
                        myGraphDeferredClick.done(function (ctrlPressed) {
                            if (item) {
                                drillDownUsing(graph, $.parseJSON(postDataCreator(item)), ctrlPressed);
                            }
        ...
        widgets: function () {
            ...
            function addNewWidget(event) {
                ...
                var ajaxOptions = {
                    url: routeData.AddWidget, //'/Widget/AddWidget',
                    type: 'POST',
                    data: JSON.stringify({
                    ...
                    success: function (data) {
                        ...
                        if (toomanywidgets != null) { // too many widgets
                            ...
                            $rhea.prepareNewContent(newContent);
                            button.parent().attr('style', 'display: none;'); // hides the button and the surrounding list item element
            ...
            root.find('.zeus-widget-data-context-list  > div > div').on("click", function () {
                var headers = {};
                headers[headerTypes.Ajax] = true;
            ...
            root.find('[' + fullDataTypes.Click + '=remove]').on("click.zeus-widgets", function (e) {
                ...
                // Add back the button to the selection list if there is one ... Unhide it.
                var listItemStyle = $('.zeus-widget-selector li[style="display: none;"]').has('td:contains("' + name +  '")');
                listItemStyle.removeAttr('style');
                    ...
                    if (numHidden != hiddenColumns.length) { // Change in hidden columns, regenrate the drop down rows
                        table.find('tbody tr:not(.responsive-row)').each(function () {
                            ...
                            if (hiddenColumns.length) {
                                ...
                                var colspan = tableHeaderRow.children(':visible').length;
                                if (colspan == 0) {
                                    colspan = tableHeaderRow.children().length;
                                }
        ...
        processCalendar: function () {
            ...
            var sessionStorageDateKey = "calendar_last_date";
            var sessionStorageViewKey = "calendar_last_view";
            var sessionStorageDayKey = "calendar_last_day";
            ...
            $(".vertical-box").each(function () {
                ...
                //get persisted day
                if (currentCalendar.attr(fullDataTypes.PersistedDay) === "true") {
                    dateOnDayView = sessionStorage[sessionStorageDayKey];
                }
                else {
                    dateOnDayView = null;
                }
                ...
                var categories = [];
                var categoryDiv = calendarDiv.find("#calendarcategories");
                if (categoryDiv != undefined) {
                    categories = categoryDiv.find("p");
                }

                var getResourcesUrl = currentCalendar.attr(fullDataTypes.CalendarGetResourcesUrl);
                var getEventsUrl = currentCalendar.attr(fullDataTypes.CalendarGetEventsUrl);

                var showResources = getResourcesUrl != undefined && getResourcesUrl !== "";

                var displaywkend = false;
                var showWeekends = currentCalendar.attr(fullDataTypes.CalendarShowWeekends);
                if (showWeekends === "true") displaywkend = true;

                var useLastDate = currentCalendar.attr(fullDataTypes.CalendarUseLastDate);
                var useLastView = currentCalendar.attr(fullDataTypes.CalendarUseLastView);
                ...
                if (useLastView === "true") {
                    ...
                    if (storedView != undefined) {
                        ...
                        if (!showResources && defaultview === "timelineDay") {
                            defaultview = "agendaDay";
                        }
                    }
                }

                var displayviews = currentCalendar.attr(fullDataTypes.CalendarDisplayViews);

                var cacheRange = 30;
                if (currentCalendar.attr(fullDataTypes.CalendarCacheRangeDays) != undefined &&
                    parseInt(currentCalendar.attr(fullDataTypes.CalendarCacheRangeDays)) > 0) {
                    cacheRange = parseInt(currentCalendar.attr(fullDataTypes.CalendarCacheRangeDays));
                }

                var cacheStart = undefined;
                var cacheEnd = undefined;
                var eventsCache = {};

                var weekendBtnString = "showWeekend";
                var filterBtnString = "showFilter";

                if (displaywkend) {
                    weekendBtnString = "hideWeekend";
                }

                if (!$("#ContainerFor-SearchCriteriaGroup1").hasClass('hidden')) {
                    filterBtnString = "hideFilter";
                }

                $("button[value='ShowWeekends']").hide();
                $("button[value='HideWeekends']").hide();
                $("button[value='ShowCriteria']").hide();
                $("button[value='HideCriteria']").hide();

                var dayBtnText = 'Day';

                if (showResources) {    //schedule view
                    dayBtnText = 'Vertical';
                }

                var calendarSettings = {
                    schedulerLicenseKey: '0215523095-fcs-1496962620',
                    header: {
                        left: filterBtnString + " prev,next today gotoButton",
                        center: "title",
                    right: displayviews + " " + weekendBtnString
                    },
                    height: "auto",
                    views: {
                        agendaWeek: {
                    ...
                    selectable: true,
                    nowIndicator: true,
                    weekends: displaywkend,

                    viewRender: function (view, element) {
                        if (useLastDate === "true") sessionStorage[sessionStorageDateKey] = moment(view.start);
                        if (useLastView === "true") sessionStorage[sessionStorageViewKey] = view.name;
                    },

                    customButtons: {
                        //custom calendar buttons here
                        gotoButton: {
                            text: 'goto',
                            icon: 'ui-icon-calendar',
                            click: function () {
                                $("#gotodate").datepicker("show");
                            }
                        },
                        showWeekend: {
                            text: '7d',
                            click: function () {
                                $("button[value='ShowWeekends']").trigger("click");
                            }
                        },
                        hideWeekend: {
                            text: '5d',
                            click: function () {
                                $("button[value='HideWeekends']").trigger("click");
                            }
                        },
                        showFilter: {
                            text: '',
                            click: function () {
                                $("button[value='ShowCriteria']").trigger("click");
                            }
                        },
                        hideFilter: {
                            text: '',
                            click: function () {
                                $("button[value='HideCriteria']").trigger("click");
                            }
                        },
                        agendaDay: {
                            text: dayBtnText,
                            click: function() {
                                    ...
                                    //goto current day of week
                                    var date = moment();
                                    var dow = date.day();
                                    var anotherDate = calendarDiv.fullCalendar('getDate');

                                    if (dateOnDayView == null) {
                                        var newDay = anotherDate.startOf('week').add(dow - 1, 'days');
                                        calendarDiv.fullCalendar('gotoDate', newDay);
                                    }
                                    else {
                                        var newDay = anotherDate.startOf('week').add(dateOnDayView - 1, 'days');
                                        calendarDiv.fullCalendar('gotoDate', newDay);
                                    }
                            }
                        },
                        timelineDay: {
                            text: "Horizontal",
                            click: function() {
                                calendarDiv.fullCalendar('changeView', 'timelineDay');
                        ...
                        next: {
                            ...
                            click: function () {
                                ...
                                if (currentView.currentRangeUnit == "day") {
                                    var selectedDate = calendarDiv.fullCalendar('getDate');
                                    dateOnDayView = selectedDate.day();
                                    if (currentCalendar.attr(fullDataTypes.PersistedDay) === "true") {
                                    sessionStorage[sessionStorageDayKey] = dateOnDayView;
                                }
                            }
                            }
                        },
                        ...
                        today: {
                            ...
                            click: function () {
                                ...
                                dateOnDayView = null;
                                if (currentCalendar.attr(fullDataTypes.PersistedDay) === "true") {
                                sessionStorage[sessionStorageDayKey] = null;
                            }
                        }
                        }
                    },
                    ...
                    events : function (start, end, timezone, callback) {
                        if (cacheStart == undefined || start < cacheStart || end > cacheEnd) {
                            cacheStart = start.add(-cacheRange, "days");
                            cacheEnd = end.add(cacheRange, "days");
                            ...
                            $.ajax({
                                    ...
                                    $(doc)
                                        .each(function () {
                                            events.push({
                                                ...
                                                popOverTitle: $(this).attr('popOverTitle'),
                                                popOverContent: $(this).attr('popOverContent')
                                            });
                                        });
                                    eventsCache = events;
                                    callback(events);
                                ...
                                error: function (xhr, status, data) {
                                    //var errorText = $.zeusValidate.getErrorInAjax(xhr);
                                    //errorText = (errorText != undefined ? errorText : 'Error occurred while loading.' + status + data);
                                    alert("There was an error fetching events. " + data);
                                    // todo stop refreshing page
                                }
                            });
                        } else {
                    ...
                    select: function (start, end, jsEvent, view, resource) {
                        ...
                        var popoverTemplate = '<div class="popover new-event" role="tooltip"><div class="arrow"></div><h3 class="popover-title"></h3><div class="popover-content"></div></div></div>';
                        ...
                        element.popover({
                            ...
                            title: function () {
                                var title = moment(start).format("D/M/YYYY H:mm a") + " - " + moment(end).format("D/M/YYYY H:mm a");
                                if (moment(start).isSame(moment(end), "day")) {
                                    title = moment(start).format("D/M/YYYY H:mm a") + " - " + moment(end).format("H:mm a");
                                }
                                if (resource != undefined) title = title + "<br />" + resource.title;
                                return title + "<button type=\"button\" class=\"close\" data-dismiss=\"popover\"  style=\"position: absolute; right:-6px; top:-6px;\" aria-hidden=\"true\">&times;</button>";
                            },
                            content: function () {
                                var html = "";
                                categories.each(function () {
                                    var url = $(this).attr(fullDataTypes.CalendarCategoryCreateUrl);
                                    var ch = "?";
                                    if (url.indexOf("?") > 0) ch = "&";
                                    url = url +
                                        ch +
                                        "start=" +
                                        moment(start).format("YYYYMMDDHHmm") +
                                        "&end=" +
                                        moment(end).format("YYYYMMDDHHmm");
                                    if (resource != undefined) url = url + "&consultant=" + resource.id;
                                    var name = this.innerHTML;
                                    html = html + "<a href=" + url + ">" + name + "</a><br />";
                                });
                                return html; //+"<input type=\"button\" value=\"Close\" aria-hidden=\"true\" onclick=\"closeme(); \">";
                            }
                        });

                        if (moment(start).format("YYYYMMDDHHmm") + moment(end).format("YYYYMMDDHHmm") == openNewDate) {
                            $(this).popover('hide');
                            openNewDate = null;
                        }
                        else {
                        ...
                        element.popover("show");
                            openNewDate = moment(start).format("YYYYMMDDHHmm") + moment(end).format("YYYYMMDDHHmm");
                    ...
                    eventRender: function (event, element) {
                        var attachedicons = "";
                        var arrayLength = event.className.length;

                        var foundicon = false;

                        for(var i = 0; i < arrayLength; i++) {
                            switch (event.className[i]) {
                                case 'attended': attachedicons = '<i class="fa fa-check" aria-hidden="true"></i> '; foundicon = true; break;
                                case 'rescheduled': attachedicons = '<i class="fa fa-chevron-circle-up" aria-hidden="true"></i> '; foundicon = true; break;
                                case 'dna-valid': attachedicons = '<i class="fa fa-check-circle" aria-hidden="true"></i> '; foundicon = true; break;
                                case 'dna-descretionary': attachedicons = '<i class="fa fa-times-circle" aria-hidden="true"></i> '; foundicon = true; break;
                                case 'dna': attachedicons = '<i class="fa fa-exclamation-circle" aria-hidden="true"></i> '; foundicon = true; break;
                                case 'dna-followup': attachedicons = '<i class="fa fa-chevron-circle-left" aria-hidden="true"></i> '; foundicon = true; break;
                                case 'dna-transfer': attachedicons = '<i class="fa fa-chevron-circle-right" aria-hidden="true"></i> '; foundicon = true; break;
                                case 'transfer': attachedicons = '<i class="fa fa-chevron-right" aria-hidden="true"></i> '; foundicon = true; break;
                                case 'dnai': attachedicons = '<i class="fa fa-times" aria-hidden="true"></i> '; foundicon = true; break;
                                case 'cancelled': attachedicons = '<i class="fa fa-ban" aria-hidden="true"></i> '; foundicon = true; break;
                                case 'create-error': attachedicons = '<i class="fa fa-minus" aria-hidden="true"></i> '; foundicon = true; break;
                                case 'flag': attachedicons = '<i class="fa fa-flag" aria-hidden="true"></i> '; foundicon = true; break;
                                case 'phone' : attachedicons = '<i class="fa fa-phone" aria-hidden="true"></i> '; foundicon = true; break;
                                case 'video': attachedicons = '<i class="fa fa-video-camera" aria-hidden="true"></i> '; foundicon = true; break;
                            }

                            //only get the first icon
                            if (foundicon) {
                                break;
                            }
                        }
                        ...
                        $(element).tooltip({
                            title: attachedicons + event.title,
                            container: "#calendar",
                            delay: {
                                show: 500, hide: 100
                            },
                        ...
                        var arrayLength = event.className.length;
                        for (var i = 0; i < arrayLength; i++) {
                            classesAttached += event.className[i] + " ";
                        }

                        var popoverTemplate = '<div class="popover ' + classesAttached + '" role="tooltip"><div class="arrow"></div><h3 class="popover-title"></h3><div class="popover-content"></div></div></div>';

                        var displayedTitle = event.popOverTitle + "<span style=\"position: absolute; right:-6px; top:-6px;\" ><button type=\"button\" class=\"close\" data-dismiss=\"popover\"  aria-hidden=\"true\">&times;</button><span>";
                        element.popover({
                            placement: "auto",
                            html: true,
                            trigger: "manual",
                            ...
                            content: event.popOverContent //+ "<input type=\"button\" value=\"Close\" aria-hidden=\"true\" onclick=\"closePopovers(); \">"
                        });
                    },
                    ...
                    eventClick: function (calEvent, jsEvent, view) {
                        ...
                        if (calEvent == openPopover) {
                            $(this).popover('hide');
                            openPopover = null;
                        }
                        else {
                        $(this).popover('show');
                            openPopover = calEvent;
                        }
                ...
                if (showResources) {
                    calendarSettings.resourceLabelText = currentCalendar.attr(fullDataTypes.CalendarResourceLabelText);
                    calendarSettings.refetchResourcesOnNavigate = false;
                    calendarSettings.resources = getResourcesUrl;
                    calendarSettings.buttonText = {
                        today: "Today",
                        agendaDay: "Vertical",
                ...
                if (currentCalendar.attr(fullDataTypes.CalendarMinDateSelectable) != undefined) {
                    var minDateSelectable = currentCalendar.attr(fullDataTypes.CalendarMinDateSelectable);
                    var maxDateSelectable = currentCalendar.attr(fullDataTypes.CalendarMaxDateSelectable);
                    calendarSettings.selectConstraint = {
                        start: minDateSelectable,
                        end: maxDateSelectable
                    };
                }

                if (useLastDate === "true") {
                    var storedDate = sessionStorage[sessionStorageDateKey];
                    if (storedDate != undefined) {
                        calendarSettings.defaultDate = storedDate;
                    }
                }

                $(calendarDiv).fullCalendar(calendarSettings);
                ...
                $("#gotodate").change(function (e) {
                    var selectedDate = moment(e.target.value);
                    dateOnDayView = selectedDate.day();
                    if (currentCalendar.attr(fullDataTypes.PersistedDay) === "true") {
                    sessionStorage[sessionStorageDayKey] = dateOnDayView;
                    }

                    calendarDiv.fullCalendar('gotoDate', e.target.value);                    
                });

                //set the day if persisted on the main calendar
                if (currentCalendar.attr(fullDataTypes.PersistedDay) === "true") {
                    var anotherDate = calendarDiv.fullCalendar('getDate');
                    var newDay = anotherDate.startOf('week').add(dateOnDayView - 1, 'days');
                    calendarDiv.fullCalendar('gotoDate', newDay);
                ...
                //setup tooltips on calendar buttons
                $(".fc-hideWeekend-button").attr('data-toggle', 'tooltip');
                $(".fc-hideWeekend-button").attr('title', 'Hide weekends');

                $(".fc-showWeekend-button").attr('data-toggle', 'tooltip');
                $(".fc-showWeekend-button").attr('title', 'Show weekends');


                $(".fc-hideFilter-button").attr('data-toggle', 'tooltip');
                $(".fc-hideFilter-button").attr('title', 'Hide Search Criteria');

                $(".fc-showFilter-button").attr('data-toggle', 'tooltip');
                $(".fc-showFilter-button").attr('title', 'Show Search Criteria');

                $(".fc-prev-button").attr('data-toggle', 'tooltip');
                $(".fc-prev-button").attr('title', 'Previous');

                $(".fc-next-button").attr('data-toggle', 'tooltip');
                $(".fc-next-button").attr('title', 'Next');

                $(".fc-today-button").attr('data-toggle', 'tooltip');
                $(".fc-today-button").attr('title', 'Show today');

                $(".fc-gotoButton-button").attr('data-toggle', 'tooltip');
                $(".fc-gotoButton-button").attr('title', 'Show date');

                $(".fc-agendaWeek-button").attr('data-toggle', 'tooltip');
                $(".fc-agendaWeek-button").attr('title', 'Week view');

                $(".fc-listWeek-button").attr('data-toggle', 'tooltip');
                $(".fc-listWeek-button").attr('title', 'All hours list view');

                if (showResources) {    //schedule view
                    $(".fc-timelineDay-button").attr('data-toggle', 'tooltip');
                    $(".fc-timelineDay-button").attr('title', 'Row per Consultant view');

                    $(".fc-agendaDay-button").attr('data-toggle', 'tooltip');
                    $(".fc-agendaDay-button").attr('title', 'Column per Consultant view');
                }
                else {
                    $(".fc-agendaDay-button").attr('data-toggle', 'tooltip');
                    $(".fc-agendaDay-button").attr('title', 'Day view');
                }
            ...
            root.find('[' + fullDataTypes.UserDefaultKey + ']').each( function() {
                ...
                function storeLocally() {
                    ...
                    if (inputElement.is('select')) {
                        ...
                        var existingSession = sessionStorage[localKey] != undefined ? $.parseJSON(sessionStorage[localKey]) : {};
                ...
                if (serverValue != undefined && serverValue != null && serverValue != '' && serverValue != '01/01/0001' && (!inputElement.is('select') || inputElement.find('option[selected]').length) ) { // Force server value into storage
                    ...
                    if (defaultVersion != undefined) {
                        // Special code for selects
                        if (inputElement.is('select')) {
                            var selectRecord = $.parseJSON(defaultVersion);
                            ...
                            if ($.parseJSON(defaultVersion)[inputElement.val()]) {
        ...
        searchModals: function () {
            ...
            root.find(".zeus-search-modal").each(function () {
                ...
                var button = $('<button type="button" class="btn-inverse btn">').text((searchModalButtonName != undefined) ? searchModalButtonName : 'Find');
                ...
                $(document).on('editable-state-change', editableState);
            ...
            root.find('button[value="Save search criteria"]').each(function () {
                ...
                button.on('click', function (event, reallySubmit) {
                    if (!reallySubmit) {
                        ...
                        modal.find('.modal-footer a').on('click.zeus-modal-dismiss', function () {
                            $rhea.dismissModalDialogElement(modal);
                        });
        ...
        processTreeview: function () {
            ...
            $('.jstree-default').not('.jstree').on("ready.jstree", function(evt, data) {
            ...
            $('.jstree-default').not('.jstree').on("open_node.jstree", function (evt, data) {
                var target_treedivid = evt.currentTarget.parentElement.id;
            ...
            $('.jstree-default').not('.jstree').on("close_node.jstree", function (evt, data) {
                var target_treedivid = evt.currentTarget.parentElement.id;
                var zid = data.node.li_attr[fullDataTypes.TreeviewNodeId];
                $('#' +target_treedivid + ' .ztv-expanded').each(function (index) {
                    if($(this).attr(fullDataTypes.TreeviewNodeId) == zid) {
            ...
            $('.jstree-default').not('.jstree').on("select_node.jstree", function (evt, data) {
                var target_treedivid = evt.currentTarget.parentElement.id;
                var zid = data.node.li_attr[fullDataTypes.TreeviewNodeId];
        ...
        treeviewRenderActionIcons: function (node, recursion, treeviewId) {
            ...
            node.children('[data-zpnfa]:not(.jstree-selflink)').each(function (index) 
            {
